...
function Search() {
  ...
  useEffect(() => {
    Axios.get('https://netease-cloud-music-api-five-roan-88.vercel.app/search/hot/detail').then(
      (res) => {
  ...
  useEffect(() => {
    Axios.get('https://netease-cloud-music-api-five-roan-88.vercel.app/toplist/detail').then(
      (res) => {
        ...
        <Swiper.Item key={index}>
          <div className=" bg-[white] rounded-[10px] w-[243px] m-[auto] ml-[20px]">
            <div className=" flex items-center h-[46px] border-[#f4f4f4] border-b-[1px]">
              ...
              <div className=" text-[12px] bg-[#e5e7eb] h-[18px] rounded-[9px] px-[7.5px]">
                播放
              </div>
            </div>
            <div>
  ...
  return (
    <div className=" bg-[#efeff0] overflow-hidden pb-[45px]">
      <div className=" h-[75px] w-[341px] flex justify-between items-center m-[auto]">
        ...
        <div>
          <input
            type="text"
            className=" w-[270px] h-[37.5px] rounded-[18.75px] border-[0px] outline-none pl-[30px] box-border"
          />
        </div>
      ...
      <div className=" px-[11.25px]">
        ...
        <div className=" flex flex-wrap  mb-[18.6px]">
          {like.map((value, index) => {
            return index < 5 ? (
              <div
                key={index}
                className=" px-[11.25px] py-[7.5px] h-[34.7px] rounded-[17.35px] bg-[white] mt-[11.25px] mr-[7.5px] shrink-0 box-border"
              >
                {value.searchWord}
              </div>
            ) : (
      ...
      <Swiper indicator={() => null} slideSize={70} defaultIndex={0}>
        {items}
      </Swiper>
    </div>
  );

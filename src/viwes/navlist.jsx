...
function Navlist() {
  ...
  useEffect(() => {
    Axios.get('https://netease-cloud-music-api-five-roan-88.vercel.app/homepage/dragon/ball').then(
      (res) => {
  ...
  const items = data.map((item, index) => (
    <Swiper.Item key={index}>
      <div className=" w-[48.79px] h-[75px] text-[12px] flex flex-col items-center justify-evenly ml-[13px] text-[#666f7d]">
        <Image
          src={item.iconUrl}
          className=" w-[48.79px] h-[48.79px]"
          style={{
